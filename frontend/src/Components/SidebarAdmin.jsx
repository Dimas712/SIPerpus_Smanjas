...
const SidebarAdmin = ({ activeMenuItem: propActiveMenuItem, profileImage: propProfileImage, userName }) => {
  ...
  return (
    <div className="flex">
      <div className={`bg-green2 text-white ${open ? 'w-72' : 'w-16'} h-screen duration-300 relative p-4 rounded-r-2xl`}>
        <div>
          <ArrowLeftCircleIcon
            className={`h-10 w-10 -right-5 absolute cursor-pointer ${!open && 'rotate-180'}`}
            stroke="#657864"
            fill="white"
        ...
        <div className="mt-8 mx-[-1rem]">
          <ul>
            <Link
              to="/dashboard"
              className={`flex items-center cursor-pointer py-2 mb-4 hover:bg-green active:bg-green3 ${open ? 'px-6' : 'px-3'} ${
                activeMenuItem === 'dashboard' ? 'bg-green' : ''
              }`}
            >
            ...
            <Link
              to="/datapenggunalist"
              className={`flex items-center cursor-pointer py-2 mb-4 hover:bg-green active:bg-green3 ${open ? 'px-6' : 'px-3'} ${
                activeMenuItem === 'datapenggunalist' ? 'bg-green' : ''
              }`}
            >
            ...
            <li className={`flex flex-col cursor-pointer py-2 mb-4 ${open ? 'px-6' : 'px-3'} hover:bg-green `}>
              ...
              {dataSubMenuOpen && (
                <ul className={`mt-3 ${open ? '' : 'hidden'} `}>
                  <Link
                    to="/databukulist"
                    className={`flex items-center cursor-pointer py-2 mb-1 hover:bg-green2 active:bg-green3 ${open ? 'px-6' : 'px-3'}  `}
                  >
            ...
            <Link
              to="/profiladmin"
              className={`flex items-center cursor-pointer py-2 mb-4 hover:bg-green active:bg-green3 ${open ? 'px-6' : 'px-3'} ${
                activeMenuItem === 'profil' ? 'bg-green' : ''
              }`}
            >
